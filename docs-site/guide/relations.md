...
> **⚠ 关联实体请统一命名策略**：使用 `@Refine[naming: "snake"]` 时，`has_one` / `has_many` 的 `by` 外键列按**源实体**策略计算，若目标实体策略不一致会静默查不到行。请让关联双方使用相同的命名策略，详见 [实体定义 - 命名约定](../guide/entities.md#命名约定refinenaming)。
