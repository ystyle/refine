...
- **M22** `meta.cj:228-238` `typeNameToStorageType` 把未知 struct 一律映射为 Text 而非 design 的 Json——自定义类型适配未真正接线到宏。
  ...
  - **✅ Json 端到端读写已落地（2026-08-05，M8-Json）**: 采用 **stdx.encoding.json.stream**（`JsonSerializable`/`JsonDeserializable<T>` 接口，编译期强制，无运行时注册表）。`src/json_util.cj` 新增 `jsonToString<T>`/`stringToJson<T>` 泛型 helper；宏层写路径对 Json 兜底字段生成 `jsonToString(entity.f)`（序列化后 String 走 dispatchSet）、读路径 `stringToJson<F>(s)`（rowMapper 反序列化）。struct 未实现接口 → 编译失败（比运行时注册表更强的保障）。附带修复：PG batchUpdate Json 列 cast（`?::JSONB`，CASE 表达式参数无法从 jsonb 目标列反向推断 → 42804；cast 跟随有效存储类型，`@Field[Json]` 覆盖 String 同样处理）。真实 DB roundtrip：PG/MySQL/MariaDB 全链路（save/update/batchSave/batchUpdate/upsert），SQLite mock 契约验证。测试净增 21（json_util 5 + JsonStructField 11 + SQLite mock 2 + 集成 3）全绿。设计见 `docs/plans/2026-08-04-m8-json-typeadapter.md`。
