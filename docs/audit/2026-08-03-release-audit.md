...
- **R-M19** MySQL `VALUES(col)` 已废弃（dialect_mysql.cj:47，8.0.20 起废弃，应迁 `INSERT ... AS new`）。
  ...
  - **✅ 已解决（2026-08-04，R-M19 收尾）**：新增 `MariaDBDialect <: MySQLDialect`（dialect_mariadb.cj），仅覆盖 `upsertSQL` 退回 `VALUES(col)` 老语法（MariaDB 不识别 `AS new`，ERROR 1064）+ `name()`="mariadb"，其余继承 MySQLDialect（反引号/dataTypeOf/MySQLMigrator）。`MySQLDialect` 改 `open`（name/upsertSQL 标 open）。`Refine.open` / `DatabaseConfig.toRefine` 对 mariadb 驱动建连执行 `SELECT VERSION()` 探测（`parseServerVersion`：含 "MariaDB" → MariaDBDialect，否则 MySQLDialect；探测失败回退 MySQLDialect 不阻断打开；URL 协议不可靠——mariadb:// 可连真实 MySQL）。真实 MariaDB 11.8.8 容器（daocloud 镜像，3307）验证：探测识别 + plain upsert roundtrip + versioned upsert `VALUES(version)+1` 自增（修复前 ERROR 1064 路径）。新增 dialect_mariadb_test（12 例）+ mariadb_integration_test（3 例），MySQL 9.7 AS new 回归全绿。设计见 `docs/plans/2026-08-04-r-m19-mariadb-dialect.md`。手动覆盖方言入口记档（YAGNI）。
